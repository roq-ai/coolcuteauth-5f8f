...
import { getAuditLogs } from 'apiSdk/audit-logs';
import { AuditLogInterface } from 'interfaces/audit-log';
import { AuditLogListPage as ShowCaseList } from 'pages/audit-logs';

function DashboardPage() {
  const { session } = useSession();

  const securitySettingStat = useSWR<PaginatedInterface<SecuritySettingInterface>>(
    () => `/security-settings`,
    () => getSecuritySettings({ limit: 1, offset: 0 }),
  );

  const auditLogStat = useSWR<PaginatedInterface<AuditLogInterface>>(
    () => `/audit-logs`,
    () => getAuditLogs({ limit: 1, offset: 0 }),
  );

  const statResults = [
    {
      title: 'Security Settings',
      result: securitySettingStat?.data?.totalCount,
      isLoading: securitySettingStat.isLoading,
    },
    {
      title: 'Audit Logs',
      result: auditLogStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`audit-logs`} passHref legacyBehavior>
                  <Button
